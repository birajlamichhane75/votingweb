...
import { Candidate, Contestants, Coupon, VotingCampaignStage } from '@/utils/schema/ApiInterface'
...
const  CampaignStageIndividual : React.FC<{id : string | null}> = ({ id }) => {
    ...
    const  {all_campaign_stages_Data, candidates_by_voting_stages_data } = useAppSelector((state : RootState) => state.VotingCampaignStages);
    const CampaignStageData : VotingCampaignStage = all_campaign_stages_Data.fulfilledResponse?.data?.filter((data : VotingCampaignStage) => data.id === id)?.at(0)
    const CandidateStageData : Candidate[] = candidates_by_voting_stages_data.fulfilledResponse?.data.rows;
                ...
                <div className='xl:w-[78rem] mx-auto'>
                    <div className='flex md:flex-row flex-col-reverse gap-8 md:gap-16 grow'>
                        <div className='flex-3 grow'>
                            <h2 className='font-[500] text-lg md:text-2xl mb-1 md:mb-3'>Information</h2>
                        ...
                        <div className='w-fit flex flex-col md:items-center items-start gap-5 flex-1'>
                            <div>
                                <p className='font-[500] text-lg md:text-2xl mb-1 md:mb-5 md:text-center'>Ends In</p>
                                <DynamicCountDown targetDate={new Date(CampaignStageData.endDateTime)} isBlue={true}  />
                            </div>
                            <div className='w-full'>
                                <h3 className='font-[500] text-lg md:text-2xl mb-3'>Contestants</h3>
                                <div className='flex flex-col gap-4'>
                                    {
                                        CandidateStageData?.length != 0 && 
                                        CandidateStageData?.map((contestant:Candidate) => {
                                            return (
                                                <ContestantDetailButton candidate={contestant} key={contestant.id} />
                                            )
                                        })
                                    }
                                    {
                                        CandidateStageData?.length == 0 && (
                                            Array.from({length:5}).map((_ , index) => {
                                                return(
                                                    <SkeletonCandidate isLoaded={CandidateStageData.length == 0} key={index} />
                                                )
                                            })
                                        )
                                    }
