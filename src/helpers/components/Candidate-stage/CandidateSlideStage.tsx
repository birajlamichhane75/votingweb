...
const ContestantSlideStage : React.FC<{params : Params}> = ({ params }) => {
    ...
    const {all_candidates_by_campaign_id_data} = useAppSelector((state: RootState) => state.Candidates); // Access state.Candidates
    const {isPending , isFulfilled , isRejected ,fulfilledResponse} = all_candidates_by_campaign_id_data;
        ...
        <>
            <CommonSection name='candidate-slide-stage bg-[var(--pagebg)]'>
                ...
                <div className='competition-slider px-2 sm:px-0 flex justify-between items-baseline relative'>
                    ...
                    >
                        {
                            (!isPending && !isRejected && isFulfilled && CandidatesData.length > 0) &&(
                                CandidatesData.slice(0,8).map((candidate:Contestants, index:number) => {
                                return (
