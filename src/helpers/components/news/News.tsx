                            ...
                            News?.map((news, index) => {
                                return (
                                    <SwiperSlide key={index}>
                                        <div className='px-[.5rem] py-[.5rem]  border-1 border-[var(--border-color)] rounded-md'>
                                            <Link href={`/newspage?id=${news.id}`} key={index} className='flex flex-col h-full gap-[1rem]'>
                                                <div className='w-full h-[15rem] border-1'>
                                                    <Image src={process.env.NEXT_PUBLIC_AWS_URI + news.image} alt={news.title} height={500} width={900} className='h-full w-full object-contain' />
                                                </div>

                                                <div className='flex flex-col gap-3'>
                                                    <div className='flex items-center'>
                                                        <Image src="/image/home/contest.png" height={500} width={900} alt='image' className='h-[2rem] w-[2rem] object-cover rounded-full' />
                                                        <span className='flex h-fit'>
                                                            <span className='px-[5px]'><p className='paragraph text-[var(--blue)]'>Rajiv Jaiswal</p></span>
                                                            <p className='text-[var(--blue)]'>|</p>
                                                            <span className='pl-[5px]'><p className='paragraph text-[var(--light-text-color)] '>15 May, 2024</p></span>
                                                        </span>

                                                    </div>
                                                    <h3 className='text-[14px] md:text-[16px] font-semibold text-[var(--blue)] line-clamp-2'>{news.title}</h3>
