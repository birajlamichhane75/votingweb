...
const stripeInstance = loadStripe(process.env.NEXT_PUBLIC_STRIPE_SECRET_KEY as string);
...
export default function PaymentModal({ couponTransactionData }: { couponTransactionData: GetClientSecretInterface }) {
  
  const { isOpen, onOpen, onClose } = useDisclosure();
  const { token , x_api_key } = useAppSelector((state: RootState) => state.Auth);
  const {current_coupon_transaction_data} = useAppSelector((state : RootState) => state.Coupons)
  const [loading, setLoading] = useState(true);
  const [responseIntent , setIntentResponse] = useState<intentResponse | null>(null);
  ...
  useEffect(() => {
    async function fetchIntent() : Promise<void>{
        try{
          ...
          const response = await dataService.postData('/coupon-transaction', couponTransactionData, token);
                            ...
                            <ModalBody className="flex items-center justify-center">
                              ...
                              <section className="">
                                <div className="mx-auto">
                                    {
                                      (!loading && responseIntent) && <Elements
                                      options={{
