...
function Notification({isLoggedIn} : NotificationProps) {
    ...
    const { all_notifications_data} = useAppSelector((state : RootState) => state.Notifications)
    ...
    async function fetchNotification (){
        try{
            ...
            if(!x_api_key){
                const xApiKey = await axios.get(`${process.env.NEXT_PUBLIC_VOTING_IDENTITY_URI}/x-api-key/${orgID}`);
    ...
    return (
        <Popover placement="bottom" showArrow offset={10} shouldBlockScroll={true}>
            <PopoverTrigger>
                <button className="border-none outline-none" disabled={all_notifications_data} title="notifications">
                ...
                { isLoggedIn ? (
                    <div className="notification py-1 w-full max-h-[16rem] overflow-auto">
                        <div className="flex flex-col gap-2 w-full">
                            {
                                all_notifications_data && all_notifications_data.map((item : NotificationInterface, index : number) => (
                                    <div key={index} className="bg-white rounded-xl pl-3 pr-2 py-3 shadow">
                                        <div className="flex justify-between items-center">
                                            <h1 className="font-[500] font-secular text-xs text-[var(--blue)]">{item.title}</h1>
                                            <p className="text-xs font-[500]">{item.inserted.split('T').at(0)}</p>
                                        </div>
                    ...
                    <div className="px-3 py-4 flex flex-col gap-3">
                        ...
                        <p className="text-lg text-[var(--gray)] font-[500]">Not a user? <Link href='/login' className="text-[var(--blue)]"> Register</Link ></p>
                    </div>
                )
            }
            </PopoverContent>
