...
import { ESEWA_SCD, ESEWA_TEST_PID, ESEWA_URL } from "@/utils/constants/constants";
import { Payment } from "@/utils/methods/payment";
import { RootState, useAppDispatch, useAppSelector } from "../hooks/useStoreHooks";
import { Coupon, Distribution, GetClientSecretInterface } from "@/utils/schema/ApiInterface";
...
export default function Confirm({isDisabled , paymentMethod , votesPerCandidate , coupon}  
    : {isDisabled : boolean , paymentMethod :'ESEWA' | 'STRIPE' , votesPerCandidate : { [candidateId: string]: number } , coupon : Coupon}) 
    {
        ...
        const distribution = Object.entries(votesPerCandidate).map(([candidateId, votes ]) => ({
            candidateId,
            votes
          }));
        ...
        return (
            <>
                <button
                    onClick={handleOpen}
                    className={`py-2 px-6 bg-[var(--btncolor)] text-white rounded-lg font-[500] openpayment ${isDisabled ? 'disable' : true}`}
                    disabled={isDisabled}
                >
                    Buy
                </button>
                    ...
                    <ModalContent className="px-5 py-5">
                        <ModalBody className="flex items-center justify-center">
                            <div className="w-[100%] md:w-[60%] mx-auto text-center">
                                <h3 className="text-xl md:text-3xl font-[500] text-[var(--blue)] font-secular">Are you sure you want to buy?</h3>
                                <p className="w-[100%] md:w-[40%] mx-auto font-[500] text-[var(--black)]">Clicking Confirm will send a purchase request from your account.</p>
                                <div className="flex gap-5 text-center justify-center mt-5">
                                    ...
                                    <button className="px-4 py-2 bg-[#DF383033] text-[#DF3830] rounded-lg font-[700]" onClick={onClose}>Cancel</button>
                                </div>
                            </div>
                        </ModalBody>
                    </ModalContent>
