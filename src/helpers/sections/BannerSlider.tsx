        ...
        <>
            <section className="Hero-section -has-dynamic-timer -is-hero">
                <div className='h-fit'>
                    ...
                    <div className={`sliderbanner w-full ${campaignID && individualCampaign ? 'h-[30vh] md:h-[40vh]' : 'min-h-[40vh] sm:min-h-[60vh]'} text-[#eeeeee] flex`} >
                        <Swiper
                        ...
                        >
                            {
                                (!isPending && !isRejected && BannerData?.length > 0) ?  (
                                    BannerData.map((bannerdata: VotingCampaign, index: number) => {
                                    return <>
                                        <SwiperSlide key={index}>
                                            <Image key={index} src={process.env.NEXT_PUBLIC_AWS_URI + bannerdata.banner} height={1800} width={1800} alt="Banner" className='h-full w-full object-cover' />
                                        </SwiperSlide>
                                    </>
                                ...
                                Array.from({ length: 3}).map((_, index: number) => {
                                    return (
                                        <SwiperSlide key={index}>
                                            <SkeletonBanner isloading={(isPending || isRejected || BannerData?.length == 0) || !BannerData} />
                                        </SwiperSlide>
                                    )
                                })
                            )
                            ...
                            <div className="controller_wrapper">
                                <div className='absolute top-[40%] translate-y-[-50%] left-0 z-10 flex w-full justify-between items-center py-[1rem] px-[1rem]'>
                                    <div className="back-btn b-back"><FaChevronLeft className='text-2xl' /></div>
                                    <div className="forward-btn b-forward"><FaChevronRight className='text-2xl' /></div>
                                </div>
                            </div>
                        </Swiper >
