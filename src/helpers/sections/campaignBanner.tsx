...
const CampaignBanner : React.FC<{campaignID? : string}> = ({ campaignID}) => {
    ...
    const { all_upcomming_campaign , campaign_by_id } = useAppSelector((state: RootState) => state.VotingCampaign); // Access state.Products.all_products_data directly
    const {fulfilledResponse , isFulfilled ,isPending , isRejected} = all_upcomming_campaign;
    const BannerData:VotingCampaign = fulfilledResponse?.data.rows[0]
    const individualCampaign : VotingCampaign = campaign_by_id.fulfilledResponse?.data
    ...
    return (
        <>
            <section className="Hero-section -has-dynamic-timer -is-hero">
                ...
                <div className={`banner w-full ${campaignID && individualCampaign ? 'h-[30vh] md:h-[40vh]' : 'min-h-[40vh] sm:min-h-[60vh]' } text-[#eeeeee] flex`} 
                        style={{
                            backgroundImage: (campaignID && individualCampaign) ? `url(${process.env.NEXT_PUBLIC_AWS_URI + individualCampaign.banner})` 
                            : `url(${process.env.NEXT_PUBLIC_AWS_URI + BannerData?.banner})` 
                        }}
                        ...
                        <div className='md:max-w-[598px] max-w-[90%] lg:max-w-[60%] w-[100%] mx-auto flex flex-col items-center justify-center banner-content'>
                                    ...
                                    <div className='flex flex-col items-center gap-2 py-10'>
                                        <div>
                                            <h1 className='font-secular font-extrabold text-[20px] md:text-[30px] leading-[30px] text-center 
                                             text-white'>{individualCampaign.title}</h1>
                                        </div>
                                        <div className='flex gap-1 items-center'>
                                            ...
                                            <p className='page-top-path text-[--c-secondary]'>{individualCampaign.title.split(' ').at(0)}</p>
                            ...
                            {
                                (!campaignID && isFulfilled && !isPending && !isRejected && BannerData) && (
                                    <>
                                        <div className='text-center'>
                                            <Image src="/image/home/title.png" height={90} width={90} alt="title" className='object-cover w-[30px] h-[30px] md:w-[100%] md:h-[100%] mb-[10px]' />
                                        </div>
                                        <div className='text-center flex flex-col mb-[20px]'>
                                            ...
                                            <p className='font-ubuntu text-[16px] md:text-[20px] font-[400] text-white'>{capitalize(BannerData.description)}</p>
                                        </div>
