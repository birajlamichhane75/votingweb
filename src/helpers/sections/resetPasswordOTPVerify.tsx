...
const ResetPasswordOTPverify : React.FC = () => {
    ...
    const submitHandler = async (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        try{
            ...
            const response = await dataService.patchData('users/verify-reset-token',{resetPasswordOtp : ArrayOTPtoString , email })
            if(response.success){
                dispatch(AuthSlice.actions.setResetPasswordToken(response.data?.token));
